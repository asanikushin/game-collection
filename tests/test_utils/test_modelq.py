...
def test_index_parser():
    ...
    value = Index.parse_from_header("id,name,year,categories,families,max_players,mechanics,min_players")
...
def test_batch_csv():
    ...
    value = BatchElement.from_row('307679,Billionaire,1973,Economic,,4,Auction/Bidding,2', index)
    assert value == BatchElement('Billionaire', 'Economic', 2, 4)

    value = BatchElement.from_row('145,Metropolis,1984,City Building | Negotiation,,5,,2', index)
    assert value == BatchElement("Metropolis", "City Building", 2, 5)

    value = BatchElement.from_row('22044,ZIN,1994,Abstract Strategy', index)
    assert value == BatchElement("ZIN", "Abstract Strategy", 0, 0)

    value = BatchElement.from_row('22044,ZIN,1994,Abstract Strategy,,,,4', index)
